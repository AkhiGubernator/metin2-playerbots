...
def main(root):
    ...
    edit(p,
         ...
         '\t\t\t// 12 September). What a keeper can get is the pillar of light every\n'
         '\t\t\t// client draws for a SPECIAL_EFFECT packet, sent to the searcher\n'
         '\t\t\t// alone, on every keeper found.\n'
         '\t\t\tif (ch->GetDesc())\n'
         '\t\t\t{\n'
         '\t\t\t\tTPacketGCSpecialEffect effect{};\n'
         '\t\t\t\teffect.header = HEADER_GC_SEPCIAL_EFFECT;\n'
         '\t\t\t\teffect.type = SE_LEVELUP_ON_14_FOR_GERMANY;\n'
    ...
    edit(p,
         ...
         '\t\t\t\tch->ChatPacket(CHAT_TYPE_INFO, "Stragany botow z tym towarem: %d - kazdy oznaczony kolumna swiatla.",\n'
